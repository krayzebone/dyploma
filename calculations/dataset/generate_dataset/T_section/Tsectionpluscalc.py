...
def generate_random_section() -> Dict[str, float]:
    """Generate random section parameters with validation."""
    while True:
        ...
        hf = np.random.uniform(100, 500)
        cnom = np.random.uniform(20, 60)
        fi_str = np.random.choice([8, 10, 12, 14, 16])
        
        if hf <= h and bw <= beff:
            return {
                "MEd": MEd, "beff": beff, "bw": bw, "h": h, 
                "hf": hf, "cnom": cnom, "fi_str": fi_str
            }
...
def main():
    ...
    pbar = tqdm(total=len(POSSIBLE_FCKS)*len(POSSIBLE_FIS), 
               desc="Processing fck/fi combinations")
    ...
    for fck in POSSIBLE_FCKS:
        for fi in POSSIBLE_FIS:
            ...
            while found < TARGET_PER_PAIR and attempts < MAX_ATTEMPTS:
                attempts += 1
                section = generate_random_section()
                
                solution = evaluate_section(**section)
                
                if solution and solution["fck"] == fck and solution["fi"] == fi:
                    rows.append(solution)
                    found += 1
            
            pbar.update(1)
            pbar.set_postfix({"fck": fck, "fi": fi, "found": found})
    ...
    if rows:
        ...
        df.to_parquet("Tsection_balanced30.parquet", index=False)
        print(f"\n✅ Saved {len(df)} rows to 'Tsection_balanced.parquet'")
    else:
