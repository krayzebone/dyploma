...
# ============================================
df = pd.read_parquet(r"dataset_files\Tsectionplus\Tsection1_transformed.parquet")

features = ["MEd", "beff", "bw", "h", "hf", "fi", "fck", "cnom", "fistr", "a1", "d", "cost", "As1", "As2"]
target = ["As1", "cost"]

X = df[features].values   # shape: (n_samples, 7)
y = df[target].values.reshape(-1, 2)     # shape: (n_samples, 1)
