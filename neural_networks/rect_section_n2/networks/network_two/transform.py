...
def recip(x, eps=0.0):
    ...
    if eps:                       # protect against zeros if you need to
        x = np.where(np.abs(x) < eps, np.sign(x) * eps, x)
    return 1.0 / x

recip_inverse = recip             # 1/x is its own inverse

# ============================================
# 2.  Central configuration  (unchanged)
# ============================================
DATA_CONFIG = {
    'filepath': r"neural_networks\rect_section_n1\dataset\dataset_rect_n1_test5_40k.parquet",
...
df_lambda_probe = pd.read_parquet(DATA_CONFIG['filepath']).iloc[:40000]
...
def make_box_cox(lmbda):
    return lambda x: box_cox_transform(x, lmbda)

def make_box_cox_inv(lmbda):
    return lambda y: box_cox_inverse(y, lmbda)


def make_yeo(lmbda):
    return lambda x: yeo_johnson_transform(x, lmbda)

def make_yeo_inv(lmbda):
    return lambda y: yeo_johnson_inverse(y, lmbda)

# transformations (unchanged)
TRANSFORMATION_CONFIG = {
    'features': {
        f: {'transform': make_yeo(yj_lambda), 'inverse_transform': make_yeo_inv(yj_lambda),
            'scaler': StandardScaler(), 'epsilon': 1e-10}
        for f in DATA_CONFIG['features']
    },
    'target': {
        'transform': make_yeo(yj_lambda), 'inverse_transform': make_yeo_inv(yj_lambda),
