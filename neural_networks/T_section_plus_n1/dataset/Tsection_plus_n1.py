...
def find_valid_ro(beff, bw, h, hf, fi, n_min=2, n_max=200):
    ...
    for n in range(n_min, n_max + 1):
        ...
        if 0.001 <= ro <= 0.06:  # Practical limits
            valid_ro.append(ro)
...
def calc_capacity(beff: float, 
    ...
    # Input validation
    if any(v <= 0 for v in [beff, bw, h, hf, fck, fyk, fi]) or any(v < 0 for v in [n1]):
        return float('nan')
...
def calc_crack(MEqp: float, 
    ...
    Acs = beff * hf + bw * (h - hf) + acs * As1
    Scs = beff * hf * (h - 0.5 * hf) + (h - hf) * bw * 0.5 * (h - hf)
    xI = Scs / Acs
    
    JI = ((beff - bw) * hf**3 / 12 + (beff - bw) * hf * (h - xI - 0.5 * hf)**2 + bw * h**3 / 12 + bw * h * (0.5 * h - xI)**2 + acs * As1 * (xI - a1)**2)
    ...
    discriminant = acs * As1 * (As1 * acs + 2 * beff * d)
    ...
    xII1 = (- (sqrt_discriminant + acs * As1)) / beff
    xII2 = (sqrt_discriminant - acs * As1) / beff
    ...
    if xII <= hf:
        JII = beff * xII**3 / 3 + acs * As1 * (d - xII)**2
    else:
        JII = beff * hf**3 / 12 + beff * hf * (xII - hf/2)**2 + bw * (xII - hf)**3 / 3 + acs * As1 * (d - xII)**2
    ...
    depsilon = max((sigmas - kt * fctm / roeff * (1 + acs * roeff)) / Es, 0.6 * sigmas / Es)
    Wk = srmax * depsilon
...
for _ in tqdm.tqdm(range(num_iterations), desc="Running simulations"):
   ...
   # Calculate exact integer bar counts
    ...
    Mcr, Wk = calc_crack(MEqp, beff, bw, h, hf, fck, fyk, fi, fistr, cnom, As1)

    if math.isnan(Mcr) or math.isnan(Wk):
        continue

    cost = calc_cost(beff, bw, h, hf, fck, As1)

    MRd = calc_capacity(beff, bw, h, hf, fck, fyk, fi, fistr, cnom, n1)
    M_Rd = MRd / 1e6
    ...
    A_s_max = 0.06 * (beff*hf + bw*(h-hf))
    if (As1) < A_s_min or (As1) > A_s_max:
        continue

    # Check if bars physically fit
    spacing = min(20, fi)
    width_needed_tension = n1*fi + (n1 - 1)*spacing
    if width_needed_tension > 2 * bw:
        continue
...
if data_list:
    ...
    df = df.dropna()
    df.to_parquet(r"dataset_T_n1.parquet", index=False)
