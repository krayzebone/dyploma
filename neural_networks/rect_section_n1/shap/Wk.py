...
MODEL_CONFIG = {
    'hidden_layers': [
        {'units': 450, 'activation': 'relu', 'dropout': 0.000265861021267695},
    ],
    'output_activation': 'linear'
}

TRAINING_CONFIG = {
    'optimizer': Adam(learning_rate=0.00487151166866157),
    'loss': 'mse',
    'metrics': ['mse', 'mae'],
    'batch_size': 128,
    'epochs': 300,
...
# Add this function to your plotting utilities section
def plot_feature_importance(model, X_val, y_val_scaled, feature_names):
    """Calculate and plot permutation feature importance"""
    # Convert scaled validation data back to dataframe for easier handling
    X_val_df = pd.DataFrame(X_val, columns=feature_names)
    
    # Calculate permutation importance
    result = permutation_importance(
        model, X_val_df, y_val_scaled,
        n_repeats=10,
        random_state=42,
...
def plot_shap_summary(model, X_val, feature_names):
...
def main():
    ...
    importance_df = plot_feature_importance(model, X_val, y_val_scaled, DATA_CONFIG['features'])
    print("\nFeature Importance Ranking:")
    print(importance_df[['feature', 'importance_mean']].to_string(index=False))

    plot_shap_summary(model, X_val, DATA_CONFIG['features'])
