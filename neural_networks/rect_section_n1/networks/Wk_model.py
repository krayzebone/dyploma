...
def boxcox_transform(x, lmbda=None):
    """Box-Cox transformation with automatic lambda estimation if not provided."""
    if lmbda is not None:
        return stats.boxcox(x, lmbda=lmbda)
    return stats.boxcox(x)

def boxcox_inverse(x, lmbda):
    """Inverse Box-Cox transformation."""
    return stats.inv_boxcox(x, lmbda)
...
DATA_CONFIG = {
    'filepath': r"neural_networks\rect_section_n1\dataset\dataset_rect_n1.parquet",
...
TRANSFORMATION_CONFIG = {
    'features': {
        ...
        'MEqp': {'transform': log_transform, 'inverse_transform': log_inverse, 'epsilon': 1e-8},
...
OUTPUT_CONFIG = {
    'save_path': r"neural_networks\rect_section_n1\models\Wk_model",
    'visualization': {
        'max_samples': 100000,
        'histogram_bins': 100
    },
